...
function EmbeddingReactor(options) {
    ...
    if (!options.isValidParent || typeof options.isValidParent !== 'object') { throw new Error('isValidParent option is mandatory'); }
    ...
    paper.on('cell:pointerup', function (cellView, evt, x, y) {
        ...
        if (!newparent && !isValidParent(element, undefined)) {
            abort();
            return;
        }
    ...
    model.on('add', function (cell) {
        ...
        if (parent) {
            ...
            if (!isValidParent(element, undefined)) {
